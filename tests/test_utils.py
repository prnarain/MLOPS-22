...
def test_case_3(data, label, train_frac, dev_frac):
    ...
    metric = metrics.accuracy_score
    model_path = "test_run_model_path.joblib"
    mse, bias, var = bias_variance_decomp(model_path, x_train, y_train, x_test, y_test, loss='mse', num_rounds=200, random_seed=43)
    return mse

def test_case_4(data, label, train_frac, dev_frac):
    # Estimation of bias and variance using bias_variance_decomp 
    from mlxtend.evaluate import bias_variance_decomp
    x_train, y_train, x_dev, y_dev, x_test, y_test = train_dev_test_split(
    data, label, train_frac, dev_frac)
